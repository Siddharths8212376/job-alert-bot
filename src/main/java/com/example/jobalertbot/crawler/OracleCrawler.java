...
@Component
public class OracleCrawler implements CompanyCrawler {

    private static final String BASE_API_URL = "https://eeho.fa.us2.oraclecloud.com/hcmRestApi/resources/latest/recruitingCEJobRequisitions";
    private static final String JOB_DETAIL_BASE = "https://eeho.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_45001/job/";
        ...
        try {
            for (String location : criteria.getLocations()) {
                ...
                while (hasMoreInLocation && allJobs.size() < MAX_RESULTS) {
                    ...
                    String finderValue = String.format("findReqs;siteNumber=CX_45001,location=%s,selectedCategoriesFacet=300000001917356,limit=%d,offset=%d,sortBy=POSTING_DATES_DESC",
                             location, PAGE_SIZE, offset);

                    String url = String.format("%s?onlyData=true&expand=requisitionList.workLocation&finder=%s",
                            BASE_API_URL, URLEncoder.encode(finderValue, StandardCharsets.UTF_8));
    ...
    private JobPosting mapToJobPosting(JsonNode node) {
        ...
        job.setUrl(JOB_DETAIL_BASE + node.path("Id").asText());
        return job;
    }
}
